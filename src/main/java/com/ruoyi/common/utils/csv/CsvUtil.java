...
//                    }
    ...
    public List<T> importCvsByColumnName(InputStream inputStream, int noImportRowNum) throws Exception
    {
        List<T> list = new ArrayList<T>();
        this.type = Excel.Type.IMPORT;
...
//                        }
//                    }
                    CsvBindByName columnName = field.getAnnotation(CsvBindByName.class);
                    if (columnName!=null){
                        field.setAccessible(true);
                        Integer column = cellMap.get(columnName.column());
                        if (column != null) {
                            fieldsMap.put(column, field);
                        }
                    }
